...
from Load_Data import LoadData  # This is a class that will load data from a CSV file into a pandas dataframe
import numpy as np
import pandas as pd
import scipy.signal
...
class AccessData:
    ...
    def __appendTargetFeature(self):
        ...
        for row in ws.iter_rows(values_only=True):
            if row[0]+'.pl2' in self.dFrameDict.keys():
                self.dFrameDict[row[0]+ '.pl2'].append((0.1*(0.1*row[7]))/(row[6]/1000))
    ...
    def preProcessData(self):
        ...
        if self.cfg.sex == 'F' or self.cfg.sex == 'M':
            for row in ws.iter_rows(values_only=True):
                if row[0] not in excel_files:
                    ...
                    exit(1)
                if row[2] == self.cfg.sex and row[3] == self.cfg.condition and (row[0] + '.pl2') in self.pl2_files:
                    num_files += 1
        else:
        ...
        # iterate through all files and populate the pandas dataframe with power and coherence values
        for row in ws.iter_rows(values_only=True):
            if row[2] == self.cfg.sex and row[3] == self.cfg.condition or self.cfg.sex == 'A':
